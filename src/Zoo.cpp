...
void Zoo::getMeeting(Animal *a, Animal *b) {
	if (a->getClass() != b->getClass()) {
		...
		if (name_animal_dead.size() > 0)
			cout << name_animal_dead << " is dead" << endl;
	} else // they are in the same class, they create a new children
	{
		if (a->sex != b->sex) {
			cout << "A new children between " << a->name << " and " << b->name
					<< endl;
			if (a->getClass() == "Mouse") {
				pos p = getRandomPos();
				list_a.push_back(new Mouse(p));
...
void Zoo::getGrowing() {
	for (list<Animal*>::iterator it = list_a.begin(); it != list_a.end();) {
		...
		if ((*it)->age == Animal::age_dead) {
			cout << (*it)->name << " is dead because of his age " << (*it)->age << endl;
//			tab[(*it)->p.x][(*it)->p.y] = '^';
			delete &(*it);
			it = list_a.erase(it);
			continue;
